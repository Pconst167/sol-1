...
{  
  fmc_adr = addr;
  fmc_data= *(uint16 *) (0x60000000 + addr*2);
  return fmc_data;
}
...
uint16   getMR(void)
{
   return *(uint16 *) (0x60000000);
}

void     setMR(uint16 val)
{
  *(uint16 *) (0x60000000) = val;   
}
...
void	applySUBR(void)
{
	IINCHIP_WRITE(SUBR, ((uint16)SUBN_VAR[0]<<8)+(uint16)SUBN_VAR[1]);
	IINCHIP_WRITE(SUBR2,((uint16)SUBN_VAR[2]<<8)+(uint16)SUBN_VAR[3]);   
}
void	clearSUBR(void)
{
    getSIPR(&IP_VAR[0]);
