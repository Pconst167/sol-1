...
blksiz		.equ	2048		;CP/M allocation size  (was 4096)
hstsiz		.equ	512		;host disk sector size
hstspt		.equ	18		;host disk sectors/trk
...
; Use 2048 byte blocks

; First drive has two reserved tracks for CP/M
dpb0:
		.DW 72  ;SPT - sectors per track
		.DB 4   ;BSH - block shift factor
		.DB 15  ;BLM - block mask
		.DB 0   ;EXM - Extent mask   - was 1
;		.DW 1403 ; (1439-36) DSM - Storage size (blocks - 1)
                .DW 717  ; (719-2) DSM - Storage size (blocks - 1)
		.DW 127 ;DRM - Number of directory entries - 1
		.DB $C0 ;AL0 - 1 bit set per directory block
		.DB 0   ;AL1 -            "
		.DW 0   ;CKS - DIR check vector size (DRM+1)/4 (0=fixed disk)   FIXME should be 32 and update DPB in DPH
...
seldsk:
		LD	HL,$0000
		LD	A,C
		CP	1		; 16 for 128MB disk, 8 for 64MB disk, 1 for floppy
